...
export function RankingTable({ rankings, selectedCellId, onSelectCell }: RankingTableProps) {
  ...
  const filtered = rankings.filter((r) => {
    // Null-safe comparisons — treat null as 0 for filtering only
    const night = r.suhii_night ?? 0;
    const tree  = r.frac_tree  ?? 0;
    if (filter === "hot")    return night >= 2.5;
    if (filter === "cool")   return night < 1.0;
    if (filter === "canopy") return tree >= 0.2;
    return true;
  });
  ...
  const exportCSV = () => {
    ...
    const rows = rankings.map((r) => [
      r.cell_id,
      r.suhii_night != null ? r.suhii_night.toFixed(2) : "",
      r.lst_day     != null ? r.lst_day.toFixed(1)     : "",
      r.lst_night   != null ? r.lst_night.toFixed(1)   : "",
      r.frac_built  != null ? `${(r.frac_built * 100).toFixed(0)}%` : "",
      r.frac_tree   != null ? `${(r.frac_tree  * 100).toFixed(0)}%` : "",
    ]);
    ...
    link.setAttribute("download", "chhaon_cell_rankings.csv");
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-slate-800">
        <div className="flex items-center space-x-3">
          <div className="flex items-center space-x-1.5 font-bold text-slate-100">
            ...
            <span>Hotspot Triage & Cell Ranking</span>
          ...
          <table className="w-full text-left border-collapse text-[11px]">
            <thead className="sticky top-0 bg-slate-950 text-slate-400 font-semibold border-b border-slate-800">
              <tr>
                ...
                <th className="py-1.5 px-3">Night SUHII</th>
                <th className="py-1.5 px-3">Day LST</th>
                <th className="py-1.5 px-3">Night LST</th>
            ...
            <tbody>
              {filtered.map((row, idx) => {
                ...
                const night = row.suhii_night ?? null;
                const isHot = night != null && night >= 2.5;
                const isNull = night == null;
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-1.5 px-3 font-mono font-semibold text-slate-200">
                      {row.cell_id}
                    </td>
                    ...
                    <td className="py-1.5 px-3 text-slate-300">
                      {fmtPct(row.frac_built)}
                    </td>
                    <td className="py-1.5 px-3 text-slate-300">
                      {fmtPct(row.frac_tree)}
                    </td>
                    <td className="py-1.5 px-3">
                      <button
                        onClick={(e) => {
