...
export function ScenarioPainter({ cellId, cityId, onClose }: ScenarioPainterProps) {
  const [action, setAction] = useState<"add_trees" | "add_concrete" | "restore_water">("add_trees");
  ...
  const handleRunSimulation = async () => {
    ...
    setAiTrace([
      ...
      `Simulating ${coveragePct}% ${action.replace("_", " ")} on cell ${cellId}...`,
      "Evaluating P10/P50/P90 quantile uncertainty bands...",
    ]);
    ...
    try {
      ...
      setTimeout(() => {
        setAiTrace((prev) => [...prev, "Inference completed."]);
        setResult(res);
        setLoading(false);
      }, 700);
    } catch (err: any) {
    ...
    <div className="flex flex-col h-full rounded-xl border border-orange-500/50 bg-slate-900/98 p-4 shadow-2xl backdrop-blur-md text-xs text-slate-200">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-2.5 mb-3">
        <div className="flex items-center space-x-2">
          <div className="flex h-6 w-6 items-center justify-center rounded bg-orange-600 text-white font-bold text-[11px]">
            AI
          </div>
          <div>
            <h3 className="font-bold text-sm text-white">Scenario Painter Studio</h3>
            <p className="text-[10px] text-slate-400">Target Cell: {cellId}</p>
          </div>
        </div>
      ...
      {/* Intervention Action Picker */}
      <div className="space-y-3 mb-4">
        <div>
          <label className="text-[11px] font-semibold text-slate-300">Choose Planning Intervention</label>
          <div className="grid grid-cols-3 gap-1.5 mt-1.5">
            {[
              { id: "add_trees", label: "🌳 Plant Trees" },
              { id: "restore_water", label: "💧 Water Body" },
              { id: "add_concrete", label: "🏢 Build Concrete" },
            ].map((a) => (
              <button
                key={a.id}
        ...
        <div>
          <div className="flex justify-between text-[11px] font-semibold text-slate-300 mb-1">
            <span>Intervention Area Coverage</span>
      ...
      {/* AI Trace / Thinking Log */}
      {aiTrace.length > 0 && (
        <div className="bg-slate-950 p-2.5 rounded-lg border border-slate-800 text-[10px] font-mono space-y-1 mb-3">
          <div className="text-slate-500 font-bold">AI Trace:</div>
          {aiTrace.map((msg, i) => (
      ...
      {result && (
        <div className="p-3 bg-slate-950 rounded-lg border border-slate-800 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-slate-400">Projected Thermal Delta:</span>
            <strong className={`text-sm ${result.delta_T_degC.p50 <= 0 ? "text-green-400" : "text-red-400"}`}>
              {result.delta_T_degC.p50 > 0 ? "+" : ""}
              {result.delta_T_degC.p50.toFixed(2)}°C
            </strong>
          </div>
          <div className="flex items-center justify-between text-[11px]">
            <span className="text-slate-500">Uncertainty Range (P10-P90):</span>
            <span className="text-slate-300 font-mono">
              {result.delta_T_degC.p10.toFixed(2)}°C to {result.delta_T_degC.p90.toFixed(2)}°C
            </span>
          </div>
          ...
          {result.extrapolation_warning && (
            <div className="flex items-center space-x-1 text-[10px] text-amber-400 bg-amber-950/40 p-1.5 rounded border border-amber-800/40 mt-1">
              ...
              <span>Extrapolation Warning: Scenario near boundary of historical data.</span>
            </div>
          )}
        </div>
      )}
